...
import { Coffee, Maximize2, Pause, Play, RotateCcw, SkipForward, Square, TimerReset } from 'lucide-react'
...
export function FocusDock(): React.JSX.Element {
  ...
  const linkedTask = useMemo(
    () => tasks.find((task) => task.id === focusState?.taskId),
      ...
      <section className="focus-dock is-idle" aria-label="专注计时">
        <div className="focus-idle-copy">
          ...
          <div>
            ...
            <small>今天已专注 {formatDuration(today)} · {Math.round((today / goal) * 100)}%</small>
          </div>
        </div>
        <div className="focus-dock-actions">
          <button type="button" className="secondary-button" onClick={() => void startFocus('stopwatch')}>
  ...
  const remaining = focusState.durationSeconds ? Math.max(0, focusState.durationSeconds - elapsed) : elapsed
  const progress = focusState.durationSeconds ? Math.min(1, elapsed / focusState.durationSeconds) : 0
  ...
  return (
    <section className={`focus-dock is-active phase-${focusState.phase}`} aria-label="正在计时">
      ...
      <div className="focus-active-copy">
        ...
        <div>
          <span className="focus-phase">{focusState.phase === 'break' ? '休息一下' : focusState.mode === 'pomodoro' ? '正在专注' : '自由计时'}</span>
          <strong>{linkedTask?.title ?? (focusState.phase === 'break' ? '离开屏幕，活动一下' : '未关联任务')}</strong>
        </div>
      </div>
      <time className="focus-time" aria-live="off">{formatTimer(remaining)}</time>
      <div className="focus-dock-actions">
        <button
          type="button"
        ...
        >
          {focusState.status === 'paused' ? <Play size={15} fill="currentColor" /> : <Pause size={15} fill="currentColor" />}
        </button>
        {focusState.mode === 'pomodoro' ? (
          <button type="button" className="icon-button focus-control" aria-label="跳过当前阶段" onClick={() => void skipFocus()}>
            <SkipForward size={15} aria-hidden="true" />
          </button>
        ) : null}
        <button type="button" className="icon-button focus-control" aria-label="结束计时" onClick={() => void stopFocus()}>
          <Square size={14} fill="currentColor" aria-hidden="true" />
        </button>
        <button type="button" className="icon-button focus-control" aria-label="打开专注迷你窗" onClick={() => void api.desktop.toggleMiniWindow()}>
...
export function MiniFocus(): React.JSX.Element {
  ...
  const skipFocus = useNudgeStore((state) => state.skipFocus)
  const now = useNow(focusState?.status === 'running')
  const task = tasks.find((item) => item.id === focusState?.taskId)

  if (!focusState || focusState.status === 'idle') {
    return (
      <main className="mini-focus mini-idle">
        <div className="mini-brand"><span className="brand-mark"><TimerReset size={14} /></span><strong>Nudge 专注</strong></div>
        <h1>准备好安静做一件事了吗？</h1>
  ...
  return (
    <main className="mini-focus mini-running">
      <div className="mini-progress" aria-hidden="true"><span style={{ transform: `scaleX(${progress})` }} /></div>
      ...
      <div className="mini-controls">
        ...
        {focusState.mode === 'pomodoro' ? <button type="button" className="icon-button" aria-label="跳过" onClick={() => void skipFocus()}><SkipForward size={16} /></button> : null}
        <button type="button" className="icon-button mini-stop" aria-label="结束" onClick={() => void stopFocus()}><Square size={15} fill="currentColor" /></button>
      </div>
    </main>
  )
}
